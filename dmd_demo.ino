...
void loop(void)
{
   ...
   dmd.drawMarquee("Swinburne Tinkerers Guild! :)",29,(32*DISPLAYS_ACROSS)-1,0);
   long start=millis();
   long timer=start;
   boolean ret=false;
   while(!ret){
   ...
   for (byte x=0;x<DISPLAYS_ACROSS;x++) {
     for (byte y=0;y<DISPLAYS_DOWN;y++) {
       dmd.drawString(  2+(32*x),  1+(16*y), "Swinb", 5, GRAPHICS_NORMAL );
       dmd.drawString(  2+(32*x),  9+(16*y), "urne!", 5, GRAPHICS_NORMAL );
     }
   }
   delay( 2000 );
